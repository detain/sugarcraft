...
- [x] **5.2** ✅ (round 89 lane y3, revocation build) No async `Form::validateAll()` — always synchronous blocking → shipped as `Form::validateAsync(): PromiseInterface<array<string,string>>` (map, not AsyncCmd — the revoke-brief supersedes the AsyncCmd sketch) + `AsyncValidatable` interface (`Field` + `validateAsync(): PromiseInterface<?string>`) + `HasAsyncValidation` trait on Input/Text (`withAsyncValidator(\Closure)`; slot rides CarriesNonCtorState per E741). Sync validators gate first — a locally-invalid value never costs a round trip; non-promise returns / non-string resolutions reject loudly naming the key; validateAll() byte-untouched (parity pinned). Tests: Field/AsyncValidationTest 12T + FormAsyncValidateTest 7T, hermetic hand-settled Deferreds (zero timers, E646). Was: ⏭️ ruled — zero consumers (r86 product ruling, revoked by m0519)
...
- [x] **5.4** No per-field keybinding overrides — KeyMap applies to Form navigation only → Add per-field `withKeyMap()` override so individual fields can customize their key handling — ✅ LANDED y3 r89 (revoked per operator ruling): shipped as `withKeyOverrides(array)` on TextInput + TextArea (NOT named withKeyMap — the map rebinds individual keystrokes to this widget's ACTION registry, it does not swap the Form-level KeyMap object; consult order: focused guard → overrides → vim branch → built-ins, so an override always wins). Door at set time: spellings canonicalised (mods ctrl/alt/shift folded to fixed order, aliases esc/bksp/del/pgup/pgdn folded, f1..f35 + single chars), unknown spelling/action and collapsing-duplicate spellings throw Lang-keyed (fail-loud, never half-accepted). Actions: move_start/end/left/right, backspace, delete_forward, delete_to_start/end, history_up/down (TextInput-only — TextArea door rejects), noop. Input/Text field pass-throughs delegate to the inner widget (map rides the widget mutate funnel across edits). Scope law honored: input domains only, Form untouched. Tests: tests/TextInput/KeyOverridesTest.php 18T/87A with vacuity pairs (built-in really clears/deletes when unbound).

### Medium Priority Missing Features

- [x] **5.5** No input masking for credit cards/phone numbers/SSN — ✅ LANDED y3 r89 (revoked per operator ruling): `EchoMode::Mask` + `TextInput::withMask(pattern, echoChar)` / `clearMask()` — the pattern is a delimiter-less PCRE class (same convention as `restrict`) selecting WHICH characters echo as `$echoChar`; unmatched input renders verbatim (`4111 **** **** 1111`). Door mirrors the E736 1.8 compile-probe and runs with the renderer's own `/u` flags, so pattern/flag disagreement is impossible; empty pattern + compile failure throw loudly (`Invalid mask pattern`), never silently disabling. `maskedValue()` fail-CLOSES: a PCRE subject refusal (invalid-UTF-8 paste reaching the render with charLimit 0) echoes every cell rather than leaking raw bytes. `Input::withPasswordMask(?pattern, echoChar)` field upgrade (null = clear); `withPassword()` fixed-echo path byte-identical (pins in PatternMaskTest + existing EchoModeTest). Enum-widening census EchoModeTest (3→4 cases) updated in-step. Upstream huh has only all-or-nothing Password — partial masking is a SugarCraft extension. Tests: tests/TextInput/PatternMaskTest.php 12T/31A.
...
- [x] **5.15** ✅ (round 89 lane y3, revocation build) No infinite scrolling / load-more callback for ItemList — `infiniteScrolling` flag exists but no callback fires at end → shipped: ctor `bool $hasMore=false` (+ `withHasMore()`/`hasMore()`, mutate-forwarded) and new `LoadMoreMsg`; `ItemList::update()` gains a single `loadMoreEdge()` epilogue every navigation result passes through (keyboard steps, g/G jumps, PageUp/Down, infinite wrap, and the 5.14 mouse arm — one funnel, no per-arm collisions). Edge-triggered: fires only when the cursor MOVED onto the last visible item while hasMore; resting is quiet, leave-and-return re-fires; filtering keystrokes excluded (cursor parks at 0). Default false → pre-5.15 [model, null] shape byte-identical. Hosts embedding ItemList inside Select/MultiSelect do not yet see the msg (field wrappers drop inner Cmds — pre-existing shape, E744-adjacent seam). tests/ItemList/LoadMoreTest 13T. Was: ⏭ ruled — zero consumers (r86 product ruling, revoked by m0519)
