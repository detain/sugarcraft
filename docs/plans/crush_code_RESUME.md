...
🔴 **START AT §0-NOW-89 BELOW. It carries the ROUND-86 WAVE-1-CLOSED banner — five lanes v1–v5 welded (shell/layout/bits/charts build lanes + vt re-derivation) + v6 product-rulings doc; per-lib dispositions: shell CLOSED-subscope, layout CLOSED-subscope, bits SUBSTANTIAL-CLOSE, charts P2–P4 CLOSED, vt 9-row tail defined. FLOOR UNCHANGED 12,027 / 170,392 / 0F / 0E / 1 skipped (McpClientTest canary) / EXIT 0 — the weld moved ZERO sugar-crush files, exact-carry, no re-pin. ACTIONABLE BY ROW CENSUS = **1**: E736 (r87 tail: vt 9 rows + bits 2.4 + charts P5). ROUND-87 CUT PRE-APPROVED (§2): w1 vt cheap lane (6.4 carved for post-w1 ruling), w2 bits 2.4 scrollEnd, w3 charts P5 verify. HARNESS LAW IN FORCE: serials PLAIN BASH PIPE — NEVER tmux/setsid/PTY. failOnWarning UNIVERSAL (58/58) — ZERO Warnings at every gate. Durations.tsv sugar-crush-ONLY (532 rows HELD). Config-md5 at-rest truth `d96e124ee7967eb34ef479ef824231ad`. LINK CENSUS post-r86u: sugar-crush **19/19** + candy-pty **8/8** (the old 18/7 expectation is STALE).**

## 0-NOW-89. ROUND 86 WAVE-1 CLOSED — SHELL/LAYOUT CLOSED-SCOPE, BITS SUBSTANTIAL-CLOSE, CHARTS P2–P4 CLOSED, VT TAIL DEFINED — ROUND-87 CUT ISSUED — CURRENT

**Written 2026-09-17 at the round-86 wave-1 weld closeout (code tip `5c9cd321f`, master @ this closeout's docs commits). §0-NOW-88 is SUPERSEDED — every live claim it carried is re-stated here or retired with its closure; its machinery survives VERBATIM in the APPENDICES below: operating rules 1–10 (APPENDIX I), addenda -66…-73 (APPENDIX II — UNCHANGED this close), vendor & suite machinery (APPENDIX III), the K=8 standard gate (APPENDIX II §1b), environment/philosophy (APPENDIX IV), craft-rules digest (APPENDIX V).**

**The headline: ROUND 86 ran the phase-5 port-queue per the r85 decision — re-derivation-first (zany-beige-roadrunner law), one lib per lane, six lanes v1–v6, ALL reviewed APPROVE-class before merge, five picks + one companion fold, ZERO conflicts.** v1 stamped candy-shell's 12/12 plan rows + relocated the mis-filed 4.1 test (334/694, weld-corrected +3T/+7A); v2 shipped candy-layout Phase-5/6 (SolverEdgeCaseTest 10T, zero behavior bug; 1.2 STOP-class ruled; Expression seam frozen); v3 shipped sugar-bits residue (Table shrink + visibleRows memo + Tree twin; rulings: 2.4 BUILD→r87w2, 3.1 DECLINED); v4 shipped sugar-charts P2–P4 perf (render-dump md5-identical proof; 576/1445 +6T; P5 verify→r87w3); v5 RE-DERIVED candy-vt 31 rows → 22 non-actionable, 9-row tail; v6 recorded the product rulings (forms Ph5 DECLINED wholesale, shine 7.1/7.3 DECLINED, STOP-class declines). Companion fold `5c9cd321f` carried five rv-MINORs (docblock truth + three plan erratum renotes). E736 STAYS OPEN as the sole actionable survivor — r87 tail defined below. Defect queue ZERO.

### 1. THE FLOOR — THE ANCHOR, AND ITS DOMAIN

| figure | value | domain |
|---|---|---|
| **ANCHOR (carried)** | **12,027 / 170,392 / 0F / 0E / 1 skipped (`McpClientTest` canary) / EXIT 0** — serial green x2 at r86u `988696aea`, PLAIN PIPE, linked, cwd = sugar-crush; r85 BOTH waves and the r86 wave-1 weld moved ZERO sugar-crush files → carried EXACT through weld tip `5c9cd321f`, no re-pin; K=8 conservation +0 | **UNCHANGED since r86u absorption — link-domain; ±32A cold/warm `ensureFixtureRepo()` wobble band still the assertions truth** |
| lib figures at close | candy-shell **334 / 694** (+3T/+7A corrected) - candy-layout **157 / 882** (+10T v2) - sugar-bits **509 / 1,066** (+16T v3) - sugar-charts **576 / 1,445** (+6T v4) - candy-forms **1,891 / 3,164 / 0W EXACT** (shared keystone) - consumers UNMOVED zero-wobble: prompt 158/310/2S · glow 122/313 · stickers 221/476 · query 1483/4203 · tick 144/349 · boxer 187/375 · sprinkles 751/2629 - five-guard **125 / 7,362** | lib-local; every touched lib gated FULL + both repo tools rc=0 at the weld |
| chain | `0f26082fe` (r85 W2) → `b26b9989f` (r85 filemap) → `0ab8555bf` (v6 rulings) → picks `c887c1458`(v1) `86dd9807c`(v2) `2d11ab830`(v3) `7dfa8de45`(v4) `3696b7bf7`(v5) → companion `5c9cd321f` → closeout docs + filemap = ROUND-86-TIP | NEVER pushed |
...
### 2. ROUND-87 CUT (PRE-APPROVED — the r85 decision point was answered by scheduling the port lanes)

**(a) Row census = 1 survivor** (command below RE-RUN at the filemap regen; expected output verbatim):
...
OPEN  **E736** 
$   # 1 survivor: E736 — r86 wave-1 closed three lib subscopes (shell/layout/charts-P2-4) + bits substantial-close; vt tail keeps the row OPEN
```

**(b) THE WAVES:**

1. **w1 — candy-vt cheap lane** (rv-v5-endorsed cut): API batch 4.1 `focusEvents()` accessor / 4.3 root `flush()` / 4.4 root alt-screen / 12.3 `onFocusEvent` ctor → perf small 6.1 `array_values(array_map)` removal / 6.3 `cubePalette()` memo → DRY 2.3 `colorEquals` consolidation → **6.2 ONLY IF** the consumer set-ordering verification passes (rv gate: `diff()` emission order for out-of-overlap cells must be proven set-safe for candy-vcr/crush). **6.4 (scrollUpOne O(cols)) CARVED OUT** — needs a Buffer row-shift API; ruling AFTER w1 lands.
2. **w2 — sugar-bits 2.4** — `Tabs::view()` consumes stored `scrollEnd` instead of self-walking (BUILD ruling; ellipsis-variant logic at Tabs.php ~212-246 vs :83).
3. **w3 — sugar-charts P5 VERIFY** — Phase 5 rows (5.1 coverage gaps Graph::niceNumbers/graphemeWidth/Waveline/Streamline/OHLCChart/Heatmap; 5.2 stacked/pie/histogram = feature scope, judge at lane-cut) re-derived vs tree then verdict pass.

**(c) TRIGGER-WATCH ROSTER (watches are NOT rows):** carried UNCHANGED from §0-NOW-88 (CallbackAuthoredRefusalTest in-shard flake; candy-freeze zsh asymmetry; JsonStreamBackend flush-door; wish stale README badge; q15 DECSCUSR re-verify; q10 carrier-list arm; pa paste-while-modal pin; ReadPathCensus backslash blind spot; pf maxOutputTokens threading seam + ClaudeCode ceiling no-op; E696-deny-residual; updateRegistration() redirect-churn; E309 DenialKind decision; E611 tripwire; E694 step-3 / E25 re-severity gate; E655 stays VOID; LspClientDispatchPumpTest ambient; Chat.php:8266 notice-drain seam) — r86 adds: **rv-v2 Expression mutable-`$terms` freeze-seam**; **rv-v3 Confirm shared-carrier fold-when-possible** (u2 private carrier vs E741 trait); **rv-v3 Input::withValidator/mutate trait-drop family STILL LIVE** (u2 seam, Phase-8/2-domain); **rv-v4 sixel single-channel key-collapse leg + render-dump arg shape**; **rv-v5 6.4 row-shift-API ruling + vim j/k product ruling carried**; **bits Select workerPool stores-dead asymmetry**.

**(d) Rules unchanged** — operating rules 1–10 + addenda -66…-73 in force, plus the carried laws: SwallowingCatchCensusTest in every src-touching lane's gate list; whole-tree censuses SERIAL-ONLY; sibling-lib lanes run FULL suite of every touched lib + both repo tools; serials PLAIN BASH PIPE (NEVER tmux/setsid/PTY — tests/bootstrap.php:325 stdin-repair guard); docs-only lanes gate in the MAIN tree (rv-v6/v5 shape); determinism law; census stamps embed the recount COMMAND; audit live master before trusting brief-pinned SHAs; ledger-edit law (per-section split + assert-1 + numstat + far canary); durations.tsv sugar-crush-ONLY; failOnWarning 58/58 ZERO-Warnings expectation; link census 19/19 + 8/8; **resume law: blank/identical-replay report → RESUME THE SAME task_id ≤10x with disk-probes between; salvage-first audit of the tree before ANY redo (u4 ten-blank proof; rv-v1 figure catch shows salvage cuts both ways — weld re-measures, never trusts)**.

### RESTART INSTRUCTIONS — the new-session startup recipe

**Tip chain:** this closeout's docs commits (stamps/worklog/RESUME/pointer + filemap regen) sit atop code tip `5c9cd321f`; master is fast-forwarded to the final closeout tip. NEVER pushed.

1. Read this §0-NOW-89, the worklog `## ROUND 86` section, and `docs/plans/crush_code_filemap.md` (1 row = census).
2. Preflight in any fresh sandbox: `git -C /home/sites/sugarcraft status --porcelain` shows ONLY ignorable operator dirt at the tip; `php scripts/refresh-deps.php --mode=linked` FULL then EXPLICIT census **sugar-crush 19/19 + candy-pty 8/8** (post-r86u — 18/7 is STALE; incomplete linking poisons 5 InteractivePromptContainmentTest env reds; verify BEFORE briefing); config at-rest truth `d96e124ee7967eb34ef479ef824231ad`; floor gate if doubted: K=8 `bash scripts/parallel-tests.sh --durations scripts/parallel-tests-durations.tsv --out <fresh>` (~85s) or plain-pipe serial (cwd=sugar-crush).
3. CUT r87 w1 per §2(b) — vt cheap lane, ONE lib, re-derive row anchors at lane base first (6.2 consumer-verification gate is w1's first step; 6.4 stays carved pending post-w1 ruling). Never more than ONE E736 lib per wave; NEVER push; author `Joe Huss <[EMAIL]>`; no composer, no caliber, no touch of `.mcp.json`/`.sugar-crush/config*` operator files or foreign `sugarcraft-cc`.
...
5. Close r87 as r83–r86 did: worklog + stamps + `§0-NOW-90` + pointer + filemap regen; re-pin crush figures ONLY when sugar-crush files move (three rounds now proven on the zero-move EXACT-carry shape).
