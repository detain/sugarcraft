...
| 5 | SugarBits | 🟡 | 60% | Landed: `Key\Binding`/`Help`/`KeyMap`, `Help\Help`, `Spinner\Spinner` + 7 styles, `Progress\Progress`, **`Timer\Timer` + `Stopwatch\Stopwatch`** (tick-driven count-down/up, start/stop/reset, TimeoutMsg), **`Cursor\Cursor`** (Blink/Static/Hidden modes, reverse-video render), **`TextInput\TextInput`** (insert/backspace/delete/arrow/Home/End/Ctrl-A/E/U/K, multibyte-safe, char limit, password/none echo, embedded blinking Cursor, scroll window). Remaining: `TextArea`, `Viewport`, `Paginator`, `List`, `Table`, `FilePicker`. |
