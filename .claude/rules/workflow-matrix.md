...
- `ci.yml` special cases — the `sugar-crush` test + coverage cells shard through `scripts/parallel-tests.sh` (K = cores, capped at 4) from `scripts/parallel-tests-durations.tsv`, with a fail-closed conservation gate against the pinned `sugar-crush/tests/Config/Support/suite-figure.json`; coverage shards merge into `sugar-crush/coverage.xml` via `scripts/merge-clover.php`. The `path-repo-check` job ends with `php tools/check-child-lifetimes.php`. The `tools-guards` job name is pinned by `tools/tests/CheckPathReposTest.php` — rename both in the same edit.
