...
- Adding a new lib also requires: root `composer.json` (`require` + `repositories`), `.github/workflows/ci.yml` matrix `lib:` list, `.github/workflows/vhs.yml` matrix, `README.md` table, `MATCHUPS.md` row, `docs/index.html` tile, `docs/lib/<slug>.html`, `media/icons/<slug>.png`, `codecov.yml` flag + component entries.
