...
- Lookup chain: exact locale → base language (`fr-fr` → `fr`) → `en` → raw key. A single `fr.php` covers `fr-fr`/`fr-ca`/`fr-be`/`fr-ch`/`fr-lu`. Only add regional variants (`pt-br`) when wording diverges.
- Each lib has a thin `Lang::t($key, $params)` wrapper with its namespace baked in (see `sugar-wishlist/src/Lang.php`):
