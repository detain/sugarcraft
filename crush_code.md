...
- 70% of the **real provider context window** (`contextTokenLimit()` → `ContextWindow::ofBackend()`; **393,216** on the DeepSeek-V4 sglang default as of `ed57d46a`, 196,608 on MiniMax). **This line originally described a fixed 100,000-token proxy at `Chat::REMINDER_TOKEN_LIMIT`, `src/Chat.php:139`; that constant no longer exists anywhere in `src/` or `tests/`.** → a soft system-role text nudge is appended to the next turn (`src/Chat.php:3292-3298`), the real prompt still goes out.
...
`ProviderFactory`/`CompleteRequest` already carries a model name; each provider config can report its context window (SGLang's is **393,216** for the DeepSeek-V4 default and 196,608 for MiniMax as of `ed57d46a` — the value is model-aware, which is exactly why a single constant could not stand in for it). **DONE, ahead of this write-up:** `Chat::contextTokenLimit()` already returns `ContextWindow::ofBackend($this->backend)` and the reminder tier consumes it, so the constant this item proposed replacing is gone. Swap the constant `Chat::REMINDER_TOKEN_LIMIT` for a value read off the active backend's provider config at construction time, so a Claude/GPT-4o session with a 128K-200K window doesn't get warned at the same fixed proxy as a smaller local model, and vice versa.
