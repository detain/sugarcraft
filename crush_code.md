...
> **Current resume point: §0-NOW-77 (round 75 CLOSED 2026-09-13 — THREE code picks + a merge-side companion, and THE DOC-FIGURE CAMPAIGN CLOSED: ja executed E686 tranche-11 (lane `51e5b9b39` → pick `99cc68e02`, +8T, DocFigure 63→71 arms AI–AO — ARCHITECTURE 5 drifted anchors healed by DROPPING numerals, Providers-table↔availableTypes() ordered both-directional, NEW bare-anchor absence guard over ALL 13 docs pages covering both `File.php:NNN` and prose `line NNN` shapes with the HOOKS self-narrative exemption rostered bidirectionally, AGENTS_AUTHORING five prose patches incl. eleven→twelve built-ins, COMMANDS + PROMPT_ENGINEERING measured TRUE — arms only, rv-ie's "~9 prohibitions" REFUTED at exactly 6 on both surfaces); r75-rv-ja REQUEST-CHANGES 1 MAJOR — builder healed AGENTS_AUTHORING to twelve-truth but left ARCHITECTURE "eleven slot/assembler" contradicting it (builder-missed SIBLING-PAGE contradiction) — fix-lane ja2 (lane `ddf761169` → pick `332e6fdd7`, +0T/+16A) healed it AND added the AK1 cross-page AGREEMENT leg (both word-numbers == live scandir — the pair can never silently diverge again) + AI planted-mismatch leg; ja2's erratum vs rv-ja: ARCHITECTURE:31 digit row "12 built-ins + MCP bridges" EXISTS, pinned by arm H; the reviewer's WRITTEN CLOSE CONDITION verified SATISFIED against the tree (first end-to-end conditional-close) → orchestrator ruled **E686 CLOSED** — tranches 2–11 landed, every docs page armed; residuals HELD-with-reason: external prompt_expand.md §9.12 dated 6==6 (file IS tracked at the monorepo root, ie erratum E1), COMMANDS JIT table dated); jb shipped the per-id memory-ops half (lane `c79d69cfd` → pick `90b7d1ae7`, +5T/+45A; r75-rv-jb APPROVE 0C/0M): Chat::memoryLocate() resolves ids REPO-STORE-FIRST and hands back the OWNING store — /memory delete|edit now mutate the entry the prompt shows, absent repo store degrades byte-identically home-only; dispositions — edit-reachability PASS-with-disclosure, null-home-no-fold CLOSED-AS-INTENDED, agent memory tool DECLINED into minted **E694** (orchestrator ruling recorded there: bulk-clear NOT supported for project scope — per-id ops only, any repo-store bulk routing REFUSES LOUDLY, --confirm --force DECLINED; step-1 list/search grouped display → ka; step-3 agent tool GATED on E25 re-severity review); companion `02c65e6eb` healed the Bootstrap unfilteredTools() docblock ELEVEN→TWELVE (census trio 117T/6408A green; SEAM: body ELEVEN claims :6371/:6376/:6546/:6552 + README:224/:233 → ka/kb fold); re-pin `f8eba0f34`, final linked floor **11,727 / 169,276 / 0F / 0E / 1 skipped (McpClientTest canary) / EXIT 0**, NEVER pushed — the brief's expected 11,722 was an arithmetic slip (11,714+8+5=11,727, measured exact); actionable **2 BY ROW CENSUS** — OPEN 2 (E611/E694) / PARTIAL 0 / STALE-CITATION 0 / UNCERTAIN 0 (E686 CLOSED-in-place); K=8 shardcheck CONSERVATION PASS, five-guard green, Config green, Glob pair 14T/360A, DocFigure 71T/2451A, durations.tsv 506 rows HELD (zero new test files), runtime config.json md5 05480c74…2210 stable; HARNESS LAW re-proven: serial1 from a tmux PTY answered 4 extra artifact reds (stdin-pin ×2 arms — bootstrap fclose(STDIN) guarded by !stream_isatty; size-keystone — tty probe answers the live window) — detached serials MUST run `< /dev/null > log 2>&1` redirects INSIDE the tmux command (plain-pipe shape); round-76 roster per §0-NOW-77 §2 — **ka** E694 slice-A design→ship (/memory list+search project-scope grouped display, per-id ops only, bulk-clear refusal per ruling; owns src/Chat.php memory arms + Context + tests; MEMORY.md sentence + DocFigure arm in the SAME commit; carries rv-jb MINOR-1 array-shape doc), **kb** E611 verdict lane (read-only re-derive premises at tip, pick-or-drop with evidence, zero commits unless premise dead → docs-only flip; foldable seams: README:224 eleven-family prose, Bootstrap body ELEVEN claims, ForeignAgentPresetWiringTest:273 "six" comment), sandboxes cut off `f8eba0f34`; carries — E309 watch, E655 VOID, LspClientDispatchPumpTest ambient; the five-guard filter + BUILDER-GATE GUARD MANDATE + PTY-pipe suite rule + FABRICATION-VERIFICATION + RESUME-DON'T-RESTART (builders AND reviewers) + PHANTOM-REPLAY re-cut law + MERGE-AFTER-THE-FACT are MANDATORY in every brief; addenda -66…-73 unchanged — none minted this close.**
...
>
> **THE RE-VERIFICATION SWEEP HAS RUN (round 32, read-only, three lanes over Phases 1-3 / 4-6 / 7-8).**
> Every nominally-open and every recently-claimed item was measured against the tree. Verdicts are now
> inline on the items themselves; the count below is re-derived from those verdicts rather than carried
> forward. Headline: **the plan's own "62 of 75" was right by accident** — it over-counted Phase 7 by
> one and under-counted Phase 5 by one, and the two errors cancelled. The body's competing "56 of 75"
> was simply stale.
>
> Nine items the plan described incorrectly are corrected inline. The four worth knowing before you
> pick up any lane: **P5.1** quoted a one-line system prompt that has had four full sections since
> `bf3495f5`, and **P5.2** called five tool descriptions "one-clause" when all five are multi-sentence —
> either would have sent an agent to rewrite finished work. **P8.3**'s "render branch outstanding"
> rested on a grep of `src/Renderer.php` that is true, repeatable, and aimed at the wrong file: there
> are two renderers, and the branch lives in `AgentDashboardPane.php` with 21 hits. **P3.5** is the
> inverse trap — the `strlen()` it tells you to grep for is gone from `SplitLayout.php`, so a
> grep-only check reports it fixed, while the byte-length `str_pad()` at `:238` keeps the identical
> defect and no test guards it.
>
> ~~**Phase 7 is NOT complete.** Item 6's `ARCHITECTURE.md` reproduces, in its own diagram, the exact
...
**Complete, as re-derived by the round-32 sweep:** Phase 0 (all 14) · Phase 1 items 1-3 ·
Phase 2 items 1-8 · Phase 3 item 1 · Phase 4 (all 7) · **Phase 5 (all 10)** ·
Phase 6 items 1-4 · **Phase 7 items 1-5 — item 6 is PARTIAL, so the phase is NOT complete** ·
Phase 8 items 1, 2, 3, 5, 6, 7, 10, 11, 12, 14.
**Phase 2 is complete except item 9**, the deliberately-last plugin epic.

**62 of 75 items — re-derived from scratch by the round-32 sweep, not carried forward.**
The derivation, so the next reader can check it instead of trusting it: Phase 0 14 + Phase 1 3 +
Phase 2 8 + Phase 3 1 + Phase 4 7 + Phase 5 10 + Phase 6 4 + Phase 7 5 + Phase 8 10 = **62**, against
a denominator of 14+3+9+5+7+10+6+6+15 = **75**.

**Two numbers were in this document at once and both were wrong about how they got there.** The
header said 62 and the body said 56. 62 turns out to be the right total by coincidence: it counted
Phase 7 as six-done (item 6 is partial) and Phase 5 as nine-done (item 10 is fully done, 10a *and*
10b), so an over-count and an under-count cancelled. Had only one of the two errors been present the
figure would have been off by one and nobody would have noticed, because nothing re-derived it.

**Of the 13 items outstanding, 12 are deliverables and one is not.** Item 8.15 proposes no fix and
expects none, so the honest reading is **62 of 74 deliverables, plus one standing flag**. The
remaining twelve: **2.9** (the deliberately-last plugin epic) · **3.2, 3.3, 3.4** open and **3.5**
partial · **6.5, 6.6** · **7.6** partial · **8.4, 8.8, 8.9, 8.13**.

The count is not effort: Phase 2 item 4 alone was larger than all of Phase 7.

**STALE COORDINATES, from the round-32 sweep — fix your grep before you trust a line number here.**
Most citations in this document still land on what they claim. These do not, and each one would have
sent someone to read an unrelated docblock and conclude the plan was incoherent:

| cited as | actually there now | what it was supposed to point at |
|---|---|---|
| `Bootstrap.php:363-379` (item 2.6) | the `$projectTierRefusals` collector docblock | a docblock falsely claiming `ForeignSkillDiscovery` worked — **no longer exists anywhere**, because the feature is now wired and there is nothing false left to say |
| `EngineBackend.php:104-116` (item 2.6) | the `$unreapedChildren` zombie-reaping docblock, in `src/Backend/EngineBackend.php` | same; zero mentions of `ForeignSkillDiscovery` in the file |
| `Cli/Bootstrap.php:209-212` (item 2.8) | the `$trustedSettingsRoots` docblock | backend selection, which lives ~1,300 lines further down at `:1513-1539` |
| `AgentViewPane.php:112` (item 3.5) | off by three | the `strlen()` calls, now at `:96` and `:115` |
| `SplitLayout.php:238` (item 3.5) | still the padding step | but the `strlen()` named there is gone from the file; see the item |
| `src/Renderer.php` (item 8.3) | 182KB renderer with 0 `stall` hits | the dashboard render path, which is `src/Tui/Renderer.php` → `AgentDashboardPane.php` |

Checked and **exact**, which is worth recording because it is the minority: `Renderer.php:3270`/`:3296`,
`KeyBindingRegistry.php` at 611 lines, and every `file:line` in Phases 4 and 5
(`CommandParser.php`, `CommandRegistry.php`, `Runtime.php`, the five `src/Tools/BuiltIn/` descriptions). **Phase 5 IS complete.** Item **10b** (differentiate the
...
**Phase 2 item 4 IS NOW COMPLETE — this paragraph's "do not mark it complete" is superseded, and it
is kept only because it records what the remainder was.** The round-32 sweep measured all four
template forms present: `CommandLoader.php:37-46` states it outright, and `` !`cmd` `` + `@file`
landed tier-trust-gated. Bundle **C4b**, described below as outstanding, has shipped. What follows is
the historical account of the half-done state:
...
✅ **FIXED in round 33.** `HookManager.php:34` now names `confirm-rm` on `event: PreToolUse` — both
coordinates, because the key is event+name and an example naming only one of them is not an example
of the guard. The docblock also now states outright that the guard itself was never wrong (it asks
`registry->get($event, $name)` and hardcodes no list) so a future reader does not re-open the guard
looking for a bug that was only ever in the prose. **The round-33 measurement sharpened this finding:
the example was not misspelled, it was INVERTED** — `docs/HOOKS.md:118` and its table at `:130-131`
have said all along that `confirm-remove` is *accepted* and `confirm-rm` *refused*, so the docblock
used the one accepted name as its example of a refused one. Two pages of this repo stated opposite
things about the same two strings, and the correct one was the one nobody had flagged.
...
- No repo-map/symbol-graph of any kind — confirmed entirely absent; PHP-feasible sketch proposed using composer's own generated classmaps plus this repo's own `docs/MATCHUPS.md`/`PROJECT_NAMES.md`. **(§6)**
...
3. ✅ **Wire `ForeignAgentPresetRegistry::discover()`** alongside the native `AgentPresetRegistry` construction in step 1, merging results the same way `skillRegistry()` merges native + foreign skills. **(§8, §11)** — **RE-VERIFIED DONE by the round-32 sweep**. `Bootstrap::foreignAgentPresets()` calls `discover($root)`; merged in `agentRoster()` (`:1048`) foreign-FIRST so a native preset of the same name wins, reached from `agentManager()` → `chat()` (`:594`). `ForeignAgentPresetWiringTest` 14/14. One thing remains and it is NOT what this item asked for: `AgentPreset::$source` has no reader on `Agent`, so an imported preset cannot be badged as foreign in the UI (`Bootstrap.php:1019-1023` records it).

### Phase 2 — MCP + extension-mechanism wiring

1. ✅ **Pick one `McpClient` implementation** (recommend `src/MCP/McpClient.php` — richer, agent-scoped permission model, multi-server) and rename the other (`src/McpClient.php` → e.g. `ClaudeCodeMcpClient`) rather than leaving two same-named classes as a silent pick-wrong-one risk. **(§11)** — **RE-VERIFIED DONE by the round-32 sweep**. `src/McpClient.php` no longer exists; it is `src/ClaudeCodeMcpClient.php` (`6bc5218b`), and `src/MCP/McpClient.php` is the kept one. The two never actually collided under PSR-4 — the risk was a human picking the wrong `use`.
2. ✅ **Add a `Bootstrap::mcpClient($root): MCP\McpClient` builder** reading a `.mcp.json`-style config file (define the default discovery path — see Phase 6's settings work for where this should ultimately live), wrap MCP-exposed tools as `Tools\Tool` adapters, and append them into `Bootstrap::tools()`'s returned array so `EngineBackend`/`Runtime` picks them up like any built-in tool. **(§8, §11, §13)** — **RE-VERIFIED DONE by the round-32 sweep**. `Bootstrap::mcpClient()` (`:3664`) + `mcpTools()` (`:3775`), spread into `tools()` at `:4040`. Reads `.mcp.json` via `MCP_CONFIG_FILENAME`, trust-gated, `ContainedPath`-confined (`3b0ba8fe`).
3. ✅ **Construct `WorkflowEngine`/`WorkflowRegistry` inside `Bootstrap::chat()`** and pass `workflowEngine:` into `Chat`. Confirm/thread `WorkflowRegistry`'s discovery root (likely `.sugar-crush/workflows/*.yaml`, matching the `.sugar-crush/skills` convention). **(§8, §10, §11)** — **RE-VERIFIED DONE by the round-32 sweep**. `Bootstrap.php:608` passes `workflowEngine:`; `workflowEngine()` (`:705-761`) registers both `$userConfigDir/workflows` and `$root/.sugar-crush/workflows`. The discovery root this item only guessed at is exactly what shipped.
4. ✅ **Wire `CommandLoader::loadAll()` into `Bootstrap`/`Chat`**, merging file-based commands into what `Chat::slashMenuMatches()`/`CommandRegistry::filter()` search. **Build the missing template-substitution engine first or alongside** (`$ARGUMENTS`/`$1`/`` !`cmd` ``/`@file` — none of these exist yet; wiring the loader alone isn't sufficient for the feature to do anything useful). Shell-out substitution must go through ReactPHP's `Process`, not blocking `shell_exec`, per this codebase's own event-loop convention. **(§7, §8, §11)** — **RE-VERIFIED DONE by the round-32 sweep**, **and the "HALF DONE — do not mark complete" warning further up this file is now STALE.** All four template forms are implemented: `$ARGUMENTS`/`$1`-`$9` in `CommandSpec::expandTemplate()`, and the `` !`cmd` `` + `@file` forms landed tier-trust-gated (`CommandLoader.php:37-46` narrates it). `3eca66df`.
5. ✅ **Wire `HookManager::loadFromFile()`** against a real discovered config path (e.g. `.sugar-crush/hooks.yaml`) in `Bootstrap::hooks()`, after `registerBuiltIns()` — sequence after Phase 1 item 2's `ask`/`modify` exit-code extension, otherwise a newly-reachable custom hook still can't do anything but allow/deny. **(§8, §11, §13)** — **RE-VERIFIED DONE by the round-32 sweep**. `Bootstrap.php:2424-2433` builds the user-tier `hooks.yaml` path plus a trusted-project one, loaded at `:2266` after `registerBuiltIns()` (`:2193`) — the sequencing this item required.
6. ✅ **Fix `ForeignSkillDiscovery`'s wiring — the cheapest item in this entire plan.** Call `ForeignSkillDiscovery::discoverClaude()`/`discoverOpencode()` from `SkillLoader::loadAllManifests()` or `SkillManager::loadAll()`; the class, tagging, and tests already exist, only the call is missing. Also correct the two docblocks (`Bootstrap.php:363-379`, `EngineBackend.php:104-116`) that currently claim this already works. **(§11)** — **RE-VERIFIED DONE by the round-32 sweep**. The missing call is present at `src/Skills/SkillManager.php:94-95`. Closed by `a64ae782`, whose own message says it was already done. **Both docblock citations in this item are stale** — see the drift list in the status block.
7. ✅ **Add an `LspTool implements Tool`** (diagnostics/go-to-definition, backed by the already-built `LspClient`) to `Bootstrap::tools()`'s array — minimum viable wiring; a fuller integration feeding LSP diagnostics into the Edit-tool diff view is a larger follow-up. **(§8)** — **RE-VERIFIED DONE by the round-32 sweep**. `Bootstrap::lspTool()` (`:3916-3918`), imported at `:53`; `tools()`'s own docblock counts "ELEVEN entries … it was ten until LspTool was wired". The fuller diagnostics-into-Edit-diff integration this item explicitly deferred is still deferred.
8. ✅ **Swap `Bootstrap::backend()`'s `CommandBackend` for `StreamingCommandBackend`** when `$SUGARCRUSH_BACKEND_CMD` is set (`Cli/Bootstrap.php:209-212`) — `StreamingCommandBackend` already implements real per-line `$onToken` streaming for the external-command escape hatch but is never constructed; a two-line swap once its fallback behavior for non-line-buffered commands is confirmed safe. Low priority, trivial effort — bundle with whichever item above lands last. **(§8)** — **RE-VERIFIED DONE by the round-32 sweep**, **but NOT by the mechanism this item proposed.** The "two-line swap on `$SUGARCRUSH_BACKEND_CMD`" was rejected during implementation and a separate opt-in `$SUGARCRUSH_BACKEND_CMD_STREAM` added instead (`Bootstrap.php:1513-1539`). The reason is in the docblock at `:1432-1454`: the two backends speak incompatible stdout protocols (prose vs one-token-per-line), so swapping the existing variable would have silently broken every current tier-2 user. The plan's approach was corrected, not executed.
...
4. **Adopt `candy-sprinkles\Table`** for list-shaped output currently hand-`implode()`d (`/sessions`, `/agents`, MCP server list, LSP diagnostics) — land incrementally, one command's output at a time. **(§2)**
   ...
   **MEASURED 2026-08-20 — the class name is misspelled and TWO of the four targets do not exist as described:**
   - The FQN is **`SugarCraft\Sprinkles\Table\Table`** (`candy-sprinkles/src/Table/Table.php:5,37`), not `candy-sprinkles\Table`.
   - ✅ Real: `Commands/AgentsCommand.php:84-100` (byte-length `strlen()`/`substr()` columns over model-supplied text — the same defect class as the padding item above) and `Commands/McpAuthCommand.php:66-85`.
   - ❌ **`/sessions` is a MODAL WIDGET, not a list.** `Chat::handleSessionsCommand()` (`:7457`) opens a `SessionPicker` (`buildSessionPicker()` `:7496`); there is no list-shaped text to table-ise.
   - ❌ **LSP diagnostics is false twice.** The output is `json_encode(...)` into a **`ToolResult`** (`LspTool.php:359-370`) — it goes to the MODEL, not the screen — **and the surface is dead**: `LspClient::$diagnostics` (`:38`) is filled only by `handlePublishDiagnostics()` (`:446`), which has **zero `src/` callers**.
   - ⚠️ Any claim that this item "reaches deep into `src/Chat.php`" is WRONG: `Chat.php` only `ob_start()`-captures (`:6488-6527`, `:10345-10354`); the rendering lives in `src/Commands/`.
   - 🔴 **THE TRAP:** `tests/Commands/NoRawAnsiInTranscriptTest.php` guards `McpAuthCommand`/`ShareCommand` with a **source-literal regex** (`:44`), so a `Table` styled at RUNTIME sails straight past it while reintroducing the exact bug that file exists to prevent. Use **border-only — no `Style`, no `styleFunc`, no `Theme`** — and widen that test to cover `AgentsCommand.php`, which is unguarded today.
   - Scoped to the two real targets this is **~150-250 lines across two small files**, not the four-surface job the line above implies.
5. Lower priority, bundle opportunistically: fix `strlen()`-based (not cell-width-aware) padding in `SplitLayout.php:238`/`AgentViewPane.php:112` via `candy-sprinkles\Layout::joinHorizontal()`; restyle `Cli\Help::screen()` with `candy-kit`'s `Banner`/`HelpText`/`Logo::sugarcraft()`. **(§2)** — **PARTIAL, and this item is a trap for a grep-only re-verification (round-32 sweep).** `SplitLayout.php` now contains **zero** `strlen()` calls, so grepping the name this item tells you to look for reports the bug fixed. It is not: the truncation step was made cell-width-aware (`truncateToWidth()`, `:264-289`, `preg_split('//u')` + `charWidth()`) while the *padding* step at `:238` still pads with byte-length `str_pad()`, so any multibyte-but-narrow character — an accented Latin letter — still mispads. The tell-tale function name was removed and the defect class kept. `AgentViewPane.php` still has real `strlen()` at `:96` and `:115` (the item cites `:112`). Neither file has any wide-char or multibyte assertion in its tests, so this is not merely unfixed, it is **unguarded**. `Cli\Help::screen()` is still a raw heredoc; the `candy-kit` dep exists and is unused. ⚠️ **THE `:36-41` CITE IS A 24× SIZING ERROR — the heredoc is `Help.php:38-182`, 145 lines, in a 265-line file.** Re-measured 2026-08-20. **RECOMMEND DROP for the restyle half:** `candy-kit`'s primitives emit ANSI unconditionally (`Banner.php:18-35`) and `--help` is routinely piped, so a faithful restyle needs a `posix_isatty()` guard this item never mentions; and `tests/Cli/HelpTest.php:90-104` asserts a line-start regex against `screen()` that ANY SGR prefix breaks, for every flag. A cosmetic item this plan itself grades "LOW priority, not worth a dedicated PR" costing a TTY design decision plus a ~490-line test rewrite. **Record the unused `candy-kit` dep as a finding instead.** The `SplitLayout`/`AgentViewPane` padding half is unaffected and stays open.

### Phase 4 — Commands & CLI parity

1. ✅ **Make `/model` a real slash command** — flip `slashVisible: true`, add a `Chat::submit()` branch: bare form keeps today's Ctrl+P picker behavior, `/model gpt-4o` calls `Bootstrap::backendFor()` directly. Single biggest command-typing parity gap vs. both comparators. **(§7, §10)** — **RE-VERIFIED DONE by the round-32 sweep**. `CommandRegistry.php:90-101` (`slashVisible` true, `argumentHint: '[provider]'`); dispatched at `Chat.php:5498` → `handleModelCommand()` (`:5619`) → `Bootstrap::backendFor()`. `38614fa9`.
2. ✅ **Add the missing high-value built-ins**: `/help` (render `CommandRegistry::slashCommands()` as a formatted message — trivial, closes a real discoverability hole), `/clear` (wipe history without a new session id, distinct from `/new`). Scope `/init` separately as a bigger lift. **(§7)** — **RE-VERIFIED DONE by the round-32 sweep**. `CommandRegistry.php:163,169`; `Chat.php:5464-5465`. `/clear` leaves `sessionId` untouched, so it is genuinely distinct from `/new`. `38614fa9`.
...
5. ✅ **Show `argumentHint` in the "/" popup** and **extend match-highlighting to `renderSlashMenu()`** (currently only the Ctrl+P palette highlights matched characters) — both trivial, both previously-identified-but-unclosed gaps from the first research pass. **(§7)** — **RE-VERIFIED DONE by the round-32 sweep**. `renderSlashMenu()` (`Renderer.php:2884-2949`) shows the hint via `fitSlashMenuHint()` (`:2961`) and highlights matches through `Highlighter`/`MatchStyle` (`:2898-2939`) — both halves. `38614fa9`.
6. ✅ Larger/lower-priority, scope as follow-up PRs once the above stabilize: real subcommands (`mcp list`, `session list`/`delete`, `models`, `doctor` health-check distinct from the model-invoked tool, `completion bash|zsh|fish`), `--config <path>`, a 0/1/2 exit-code convention, warn-not-silently-drop on unrecognized `--output-format`. **(§5)** — **RE-VERIFIED DONE by the round-32 sweep**, **and this item was mis-scoped by its own text as a "larger/lower-priority follow-up".** It is fully shipped in `a4be8263`: `src/Cli/Subcommands.php` carries `doctor` (`:148`), `models` (`:411`), `session list`/`delete` (`:497`/`:526`), `mcp` (`:579`) and `completion` for all three shells (`:773`/`:832`/`:903`), plus `--config` (`ArgvParser.php:335`) and an `--output-format` that rejects unrecognised values instead of degrading to text at exit 0.
7. ✅ **Replace `Chat::submit()`'s `str_starts_with()` dispatch chain's first parse step with the already-built `CommandParser::parse()`** (`src/CommandParser.php`, fully tested, currently unused outside its own test) instead of continuing to hand-roll parsing a third way. Pair with adding the missing test that walks every `slashVisible: true` registry row and asserts a live dispatch handler exists for it (§7 P3) — low priority, but a natural bundle once someone is already touching this dispatch chain for the `/model` fix in item 1. **(§7, §8)** — **RE-VERIFIED DONE by the round-32 sweep**. `Chat.php:5451` really is `(new CommandParser())->parse($text)` as the first parse step, and the paired coverage test exists and passes: `SlashDispatchTest::testEverySlashVisibleRegistryRowHasALiveDispatchHandler`.

### Phase 5 — Context, cost, and prompt quality

1. ✅ **Rewrite the base system prompt** (`Runtime::buildSystemPrompt()`, one string literal today: `'You are SugarCrush, an AI coding assistant.'`) to add tool-use guidance, tone/verbosity calibration, ask-vs-act policy, and explicit security boundaries — see §12's full before/after text. Highest-leverage single change in this phase. **(§12)** — **RE-VERIFIED DONE by the round-32 sweep** — **and this item's quoted premise is FALSE.** `buildSystemPrompt()` (`Runtime.php:1272`) is not "one string literal"; its heredoc at `~:1313-1420` already carries `# Tone and style`, `# Tool use`, `# Acting vs. asking` and `# Security`. Landed in `bf3495f5`. Left standing, this line would have sent an agent to rewrite finished work — the same failure mode as the `REMINDER_TOKEN_LIMIT` quote in item 4 below.
2. ✅ **Expand the five most-used tool descriptions** (`Bash`/`Edit`/`Read`/`Grep`/`Glob` — currently one-clause each) — see §12 for drafted replacement text per tool. **(§12)** — **RE-VERIFIED DONE by the round-32 sweep** — **and "currently one-clause each" is FALSE.** All five `description()` methods in `src/Tools/BuiltIn/` are already multi-sentence with edge-case detail.
3. ✅ **Fix `App::dispatchSkill()`** to route through `Agent::systemPrompt()` instead of handing `$skill->content` straight to `CompleteRequest` — fork-context skills currently run with zero environment orientation. **(§12)** — **RE-VERIFIED DONE by the round-32 sweep**, **but the fix sits on an unreached path.** `App.php:452-513` does build an `Agent`, call `withEnvironment()` and pass `$agent->systemPrompt()` into `CompleteRequest`. However `dispatchSkill()` has **zero production callers** (its own docblock says so; backlog §C8), so the item's framing — "fork-context skills currently run with zero environment orientation" — overstates the live impact: nothing runs a fork-context skill today, oriented or not.
...
2. ✅ **Introduce a layered settings file**: `.sugar-crush/settings.json` (project, git-tracked) + `.sugar-crush/settings.local.json` (project, gitignored) + `~/.sugar-crush/settings.json` (user-global, `config.json` kept as a deprecated alias) — merged key-by-key, highest-precedence-wins, matching Claude Code's own layering model. See §13 for the full proposed schema and file-discovery precedence. Cover, in this first pass, only the fields that already have a real consumer today: `provider`/`model`/`theme`/`titleModel`/`instructions`/`disabledSkills`. **(§13)** — **DONE** (`f0585149`, with item 1). `src/Config/LayeredSettings.php`
   merges a user tier and a **trust-gated** project tier; `LAYERED_KEYS` holds ~~eight~~ **ten** keys
   and `PROJECT_TIER_KEYS` a ~~six~~ **seven**-key subset (re-measured round 32 at
   `LayeredSettings.php:202-213` and `:265-273` — item 3's own `allowedTools`/`disabledTools` work grew
   the tier and this "already verified" note was never updated: a number that travelled without its
   domain, found inside the plan's *settled* prose rather than an open item), with `userTierOnlyKeys()` **deriving** the complement so the
...
5. **Add `keybindings` remap and `statusLine` command config** — pure additive TUI features, no interaction with the permission/hook work above, can land independently at any point. **(§13)**
   ...
   **MEASURED BEFORE STARTING (2026-08-20) — this item's "no interaction with the permission/hook
   ...
   - 🟡 **The `keybindings` remap fights the registry's design.** `src/Commands/KeyBindingRegistry.php`
     ...
     `src/Tui/KeyboardHandler.php` (`:49`, `:98`, `:344`) and `src/Renderer.php` (**`:3314` `live()`, `:3340` `grouped()` — the `:3270`/`:3296` cited here were the DOC-COMMENT mentions, re-measured 2026-08-20**), or to add a static override — which leaks between tests and
     silently voids the purity the file leans on. **Decide deliberately; do not add a static setter
     by reflex.**
   - ⚠️ **THE `src/Chat.php` CONFLICT CLAIM IS FALSE and is retracted here.** `Chat.php` contains **zero** `KeyBindingRegistry` calls — its only three occurrences (`:106`, `:1407`, `:2313`) are doc-comments and one inline comment. This half does NOT read `Chat.php` and therefore does NOT conflict with a lane holding it. (The 11 `keybinding` hits in `Chat.php` are the `?` keybinding **reference overlay** — a different feature. Grepping the word rather than the symbol is what produced the false conflict.)
   - ⚠️ **`candy-kit/src/StatusLine.php` EXISTS** and is an unrelated rendering primitive. An implementer grepping `statusLine` will find it and may conclude the work is half-done. It is not.
...
3. ✅ **Add authoring guides, in priority order**: `docs/SKILLS.md` (frontmatter field reference + worked example), `docs/AGENTS_AUTHORING.md` (careful naming to avoid colliding with root `AGENTS.md` — 12-key frontmatter reference using the 3 shipped presets as annotated examples), `docs/MCP.md` (a real `.mcp.json` example — currently zero exist in-repo), `docs/PERMISSIONS.md` (all 6 modes explained, what each actually permits). **(§9)** — **RE-VERIFIED DONE by the round-32 sweep**. All four files exist and each covers what was demanded: `SKILLS.md` (181L), `AGENTS_AUTHORING.md` (232L), `MCP.md` (220L), `PERMISSIONS.md` (240L). `PERMISSIONS.md:37-68` matrixes exactly the 6 cases of the `PermissionMode` enum (`src/Permissions/PermissionMode.php:11-18`) — counted against the enum, not against the prose. `MCP.md:63-95` carries a real `.mcp.json` covering all three live server classes, and the item's "currently zero exist in-repo" is **still true** (`find . -iname '.mcp.json'` → 0). **This item's own text says "the 3 shipped presets"; there are six** — `AGENTS_AUTHORING.md:9` was correctly written against six, so only the item line was never corrected.
4. ✅ **Add `docs/HOOKS.md`, `docs/MEMORY.md`, `docs/WORKFLOWS.md`** — lift content that already exists (as a YAML comment, as CALIBER_LEARNINGS entries, as scattered prose) into real reference pages. While writing `HOOKS.md`, fix the README's stale built-in-hooks list (it omits `BashEscapeDenyHook`). **(§9)** — **RE-VERIFIED DONE by the round-32 sweep**. All three exist (369L/218L/246L) and the README fix landed: `README.md:771` now names the three `registerBuiltIns()` hooks and explains that `BashEscapeDenyHook` is registered separately by `EngineBackend::withWorktreeRoot()` (`:376`), matching `HookManager.php:105-109`. Note for anyone re-checking: the `confirm-remove` vs `confirm-rm` pair in `HOOKS.md:112-135` is a deliberate near-miss worked example, not a stale error — `ConfirmRemoveHook::name()` really does return `confirm-rm`.
5. ✅ **Add `docs/TROUBLESHOOTING.md`** sourced directly from `CALIBER_LEARNINGS.md`'s already-solved incidents, reframed from "lesson for future AI sessions" to "answer for a stuck user." **(§9)** — **RE-VERIFIED DONE by the round-32 sweep**. 320L, 15 symptom sections, framed as answers to a stuck user rather than lessons for a future session. The sweep spot-checked structure and did not trace every section back to a specific `CALIBER_LEARNINGS.md` incident.
6. **Add `docs/ARCHITECTURE.md`** expanding the README's one-diagram section — explicitly capture the "`App` wears two hats, do not retire it" warning from `CALIBER_LEARNINGS.md`, which caused a real revert-then-restore when it lived only in a gotchas file. **(§9)** — **PARTIAL, and this is why the "Phase 7 complete" claim above was wrong (round-32 sweep).** `docs/ARCHITECTURE.md` exists at 410 lines and its "`App` hosts `Chat`" section (`:113-131`) covers some of the ground, but the specific warning this item names is absent: `grep -i 'hat\|retire'` returns nothing. Worse, the diagram at `:22-24` labels `Chat` as "**the TEA Model**: init/update/view" and `App` as merely "the pane shell" — while `App` itself `implements Model` (`src/App/App.php:71`) and `bin/sugarcrush:211` passes `Bootstrap::app(...)`, not `Chat`, into `new Program(...)`. So the one document written to prevent a specific misreading currently **reproduces that misreading in its own diagram**, and that misreading is what caused the revert-then-restore recorded at `CALIBER_LEARNINGS.md:72-79`. Remaining: the two-hats/do-not-retire framing, and a diagram that names the Model `Program` actually drives. — ✅ **DONE in round 33.** The diagram at `:22-24` now names `App` as THE root TEA Model handed to `Program` *and* the engine state object `Runtime` takes, with `Chat` labelled "a TEA Model too — hosted, not the root"; the `## Chat` heading and its opening sentence no longer claim root-model status and now say which two classes implement `Model` and which one `Program` gets; and a blockquote under `### App hosts Chat` carries the two-hats/do-not-retire warning verbatim in substance from `CALIBER_LEARNINGS.md:72-79`, including that the pane layer was **wired up rather than deleted** and that the misreading caused a real revert-then-restore. **One clause of this item's own complaint was already satisfied and should not have been listed:** `ARCHITECTURE.md:115` has always read "`bin/sugarcrush` runs `Bootstrap::app()`, not `Bootstrap::chat()`", so the `new Program(...)` half was documented; the live defects were the diagram label, the "root model" sentence, and the *absent* warning. An implementer who had grepped for `Bootstrap::app` would have found `:115` and closed the item on the wrong clause — the round-32 rule (verify by domain, not by token) applying to the round-32 sweep's own write-up. Also refreshed: the page asserted `Chat.php` was "10,381 lines, measured on this checkout" when it is 10,395; the figure is now replaced by the `wc -l` that derives it, since a measured-and-stamped line count is precisely the thing this plan has watched decay four times.
...
3. ✅ **Wire `StallDetector`** into the agent dashboard once Phase 1's `AgentManager` wiring lands — the detector itself needs no changes, purely a call-site + one render branch. **(§4, §8)** — **DONE (`ef480c77`), and the "CALL-SITE HALF DONE, render branch outstanding" annotation this line used to carry was a textbook instance of this audit's signature defect (round-32 sweep).** That annotation's evidence was "zero `stall` hits in `src/Renderer.php`". The grep is true. It is still true today. It was pointed at a file that could never have held the answer: there are two renderers, and the agent dashboard's live path is `src/Tui/Renderer.php` → `AgentDashboardPane::render()`, where `stall` appears **21** times. `entries()` now reads `BackgroundSupervisor::getStallWarnings()` once per frame and threads each `StallWarning` through `sessionEntry()` → `AgentOutputState::stallWarning` → `row()`'s "⚠ stalled" and `AgentOutputPane`'s amber border; `AgentDashboardPaneTest` 38/38. A measurement can be correct, repeatable and about the wrong domain.
4. **Decide the split-pane compositor's fate** (see "Flagged for consolidation review" above) — wire it to real side-by-side agent output once `AgentManager` exposes a live-output-buffer accessor, or explicitly document it as experimental/deferred the way `AgentsPane`'s dead arm already is. **(§4)** — **STILL UNDECIDED, and the "or document it as deferred" half is only apparently satisfied (round-32 sweep).** `renderForCurrentEnvironment()`/`renderWithSplit()` (`src/Tui/Renderer.php:62`, `:105-131`) are called from `tests/Tui/SplitPaneRendererTest.php` and nowhere else. The prerequisite this item names now exists — `AgentManager::liveOutputs()` (`:341`), documented as "the multi-agent shape the tmux/iTerm2 split-pane compositor needs" — but it too is called only from tests. Crucially, the only "documentation" of the deferral is a source docblock (`src/Renderer.php:130-141`) that punts the decision to "crush_code.md Phase 8 item 4's call" — i.e. back to this very item. **Do not read that comment as closing this.** `docs/ARCHITECTURE.md:341-346` merely lists `MultiplexerSplitPane` among TUI classes and never mentions the dormancy. Per the standing rule, the outcome is wire-it or document-it-as-an-intentional-dormant-seam; neither is done.
...
6. ✅ **Record VHS demos** for the permission-prompt modal, an Edit/Write diff result, and the agent dashboard — currently only one tape exists (plain markdown reply). **(§4)** — **DONE, and the "only one tape exists" clause was stale when re-measured 2026-08-20.** `sugar-crush/.vhs/` carries **five** tapes — `permission.tape`, `diff.tape`, `agents.tape` (exactly the three this item asks for) plus `chat.tape` (the original plain-markdown reply) and `cli.tape` — backed by `examples/permission-prompt.php`, `examples/edit-diff.php` and `examples/agent-dashboard.php`. The tape headers record the measurement work behind them (grid geometry from `stty size` under vhs v0.11.0, and which beats are colour-only changes invisible to a stripped-ANSI diff). CI renders and pushes the GIFs.
...
8. **Ship a lightweight PHP-feasible repo-map** — for a single-lib `--root`, parse `vendor/composer/autoload_classmap.php`/`autoload_psr4.php` (already generated for free) or a simple declaration-line regex; for the monorepo root, parse this repo's own `docs/MATCHUPS.md`/`PROJECT_NAMES.md` (**note the `docs/` prefix — `MATCHUPS.md` is NOT at the repo root**) into a compact `<repo-map>` block, injected once per session like `<env>`. **(§6)**
9. **Fix `Grep`'s missing `InstructionFileLoader` wiring** — `Read`/`Edit`/`Glob` all surface nested `CLAUDE.md`/`AGENTS.md` on touch; `Grep` doesn't. Trivial, ~30 min. **(§6)** — **STILL OPEN, and confirmed NOT closed incidentally by item 11 (round-32 sweep).** `grep -n InstructionFileLoader src/Tools/BuiltIn/Grep.php` → empty, against `Read.php`/`Edit.php`/`Write.php`/`Glob.php` which all import it and call `$this->instructionLoader?->loadForPath(...)`. Item 11's landed work (`loadAncestorRoots()`, `src/Context/InstructionFileLoader.php:198+`) is a different concern — the monorepo-parent walk for `loadRoot()` — not the per-touch nested-file wiring inside an individual tool. The fix is the same constructor param plus one call the other four tools already have.
...
13. **Expose sub-agent spawning as a model-callable `Task` tool** so the model can decide mid-turn to delegate, matching opencode/Claude Code, rather than requiring a user-driven command. Larger effort — depends on Phase 1's `AgentManager` wiring landing first, needs a `Tool` bridging into `AgentManager::createSubAgent()`/`AgentWorkerPool` and a decision on how the sub-agent's own tool-call stream surfaces back through `ToolStarted`/`ToolFinished`. Treat as a follow-up epic once Phase 1 ships, not a single PR. **(§3)** — **STILL OPEN, correctly, and there is a name collision waiting for whoever builds it (round-32 sweep).** `src/Agents/Task.php` already exists and is an unrelated data class — "a task assigned to a teammate within a team", `final class Task` with `id`/`teamId`/`title`/`prompt` — not a model-callable `Tool`. `src/Renderer.php:114`'s docblock still states "there is no Task/Agent tool", and `AgentManager::createSubAgent()` has no caller outside tests and wiring comments.
...
15. **Note (no fix proposed yet): no file-watching for externally-changed files.** `Read`/`Edit` re-read from disk on every call so there's no stale-in-memory risk, but there's also no proactive "this file changed since you last read it" signal the way Claude Code/opencode have — nothing stops `Edit` from clobbering an external change made between a `Read` and a later `Edit` in the same turn sequence. None of the 13 angle reports sketched a concrete fix for this one; flagging it here as a known gap worth scoping separately rather than folding a half-considered design into this plan. **(§6)** — **CONFIRMED STILL REAL, and it should not sit in the "N of 75" denominator as if it were a deliverable (round-32 sweep).** `grep -n 'filemtime\|mtime' src/Tools/BuiltIn/Read.php src/Tools/BuiltIn/Edit.php` → empty; no stale-file signal exists. But this line proposes no fix and expects none, so counting it alongside items 1-14 makes the phase look one item further from done than it is. Treat the denominator as **74 deliverables + 1 standing flag**.
...
**`EnvironmentBlock`'s git snapshot is correctly one-shot per session (non-issue, positive finding).** Three `git` shell-outs (`Context/EnvironmentBlock.php`; **the `:99-106` cite is stale — `gitStatusSnapshot()` now runs `:449-470` and there are FIVE shell-outs, the two `gitDiffSection()` calls having since landed**) are memoized via `??=` for the `Runtime` instance's lifetime (`Runtime::environmentSnapshot()`, `Runtime.php:367-370`) — no repeated `shell_exec()` per turn.
...
Grepping every `getenv()` call in `src/`/`bin/` turns up nine SugarCrush-specific variables (a tenth, `SUGARCRUSH_TOOL_CALL_PARSER`, surfaced separately in §9's documentation pass and is folded into the reconciled 10-var list in §13.10), but `Help::screen()` documents only three (`SUGARCRUSH_PROVIDER`, `SUGARCRUSH_MODEL`, `SUGARCRUSH_BACKEND_CMD`) **[STALE — CLOSED. Re-measured 2026-08-20: the `:40-45` range is wrong (the env-var block runs `Help.php:107-140`) and the "only three" count is no longer true — `SUGARCRUSH_PERMISSION_MODE`, `SUGARCRUSH_BACKEND_CMD_STREAM`, `SUGARCRUSH_DISABLE_PARALLEL_TOOL_CALLS` and `SUGARCRUSH_PARALLEL_TOOL_DEADLINE` are all documented there now. Retained as the dated research record that motivated Phase 4 item 4.]**, and README.md documents four (adds `SUGARCRUSH_DISABLE_MOUSE`, `README.md:131`, in the unrelated "Mouse" section, not the env-var reference). Undocumented anywhere in README or `--help`:
...
3. **Fix the two `SUGAR_CRUSH_*` outliers and centralize env-var docs.** (Medium priority, low effort.)
   ...
   - Add a single "Environment variables" table to README.md (a peer to the "Providers" table already there at `README.md:170`) listing all 9, with one line each; extend `Help::screen()`'s env-var block (**`src/Cli/Help.php:107-140`; the `:40-45` cite was wrong — re-measured 2026-08-20**) to at least name the remaining six even if terse (`SUGARCRUSH_TITLE_MODEL`, `SUGARCRUSH_SEARCH_ENDPOINT`, `SUGARCRUSH_DISABLE_MOUSE`, `SUGARCRUSH_DISABLE_MOUSE_CLICKS`, `SUGARCRUSH_WORKTREES_DIR`, `SUGARCRUSH_SHARE_UPLOAD_URL`).
...
2. **An environment-info block now exists** — `src/Context/EnvironmentBlock.php` (**575 lines as of 2026-08-20; the "107 lines" this paragraph was written against is a dated figure, kept only to date the observation**). `EnvironmentBlock::capture(cwd, modelName)` snapshots once per Chat session (not re-polled per turn — deliberate, matches Claude Code's documented behavior, see class doc-comment). `render()` emits:
   ```
   <env>
   Working directory: ...
...
`EnvironmentBlock::gitStatusSnapshot()` **[STALE — the `:99-106` cite is wrong (now `:449-470`) and the conclusion below is CLOSED: `gitStatusSnapshot()` now appends two `gitDiffSection()` blocks, staged and unstaged, so the "no idea what changed content-wise" gap this paragraph describes no longer exists. Retained as the dated research record that motivated Phase 8 item 10.]** is the ONLY proactive git surfacing: branch, `git status --porcelain`, `git log --oneline -5`, captured once at session start and never refreshed (documented as deliberate, matching Claude Code). It does not include `git diff` (staged or unstaged), so unlike Claude Code's own environment gathering (which additionally samples a truncated diff in some contexts) the model has no idea what changed content-wise until it calls `Bash` or the git MCP server itself. Deeper git access exists only via `src/MCP/GitCommandHandlers.php` (1261 lines — `gitStatus`, `gitSnapshot`, `git_history`, `git_commits`, `git_branches`, etc.), wired in as the `'git'` MCP server type in `src/MCP/McpClient.php:118`. This is opt-in/config-driven MCP tooling, not something surfaced by default into the prompt — it's reactive (the model has to think to call it), not proactive.
...
7. ✅ **Add a proactive (truncated) `git diff` to `EnvironmentBlock`.** (Small, low-medium priority.) — **DONE.** `gitStatusSnapshot()` (`:449-470`) appends `gitDiffSection()` for staged and unstaged, byte-capped by `DIFF_MAX_BYTES` (`:148`). `EnvironmentBlock::gitStatusSnapshot()` already ran three git shell-outs; a fourth (`git diff --stat` or a byte-capped `git diff`) would close the gap with what a user typically wants surfaced ambiently, without needing the model to think to call it. Cap aggressively (e.g. a few KB) since this is prepended to every system prompt, unlike a diff fetched on-demand via a tool call. Effort: ~1-2 hours including a size-cap test.
...
Meanwhile `Chat.php:139` hardcodes `private const REMINDER_TOKEN_LIMIT = 100000;`, used at `Chat.php:3010` (`$this->compactor->shouldSendReminder($wireHistory, self::REMINDER_TOKEN_LIMIT)`) and `Chat.php:5255/5280` (context-usage percentage/token-count display, `F.CTXK`). This is the only compaction-related check that actually fires in production (per sibling finding — `ContextCompactor::shouldCompact()`/`shouldCompactForeground()`'s 85%/95% tiers are dead). It uses a flat 100,000-token proxy regardless of which provider/model is active. **RESOLVED, and this paragraph's premise no longer exists — re-measured 2026-08-20.** `Chat::REMINDER_TOKEN_LIMIT` has **zero occurrences** in `src/` or `tests/`. The reminder tier now reads the real provider window: `dispatchTurn()` takes `int $tokenLimit`, documented as the "PROVIDER-COUNTED window", supplied from `contextTokenLimit()` → `ContextWindow::ofBackend($this->backend)` (`src/Chat.php:9630-9633`), and passed to `ContextCompactor::shouldSendReminder()` at `:5119`. So the reminder fires at 70% of whatever the active model's window actually is — **1,048,570** on the DeepSeek-V4 default, 196,608 on MiniMax — which is the fix the provider-derived-limit item below proposed. **A cautionary note on this very paragraph:** while correcting it for the model switch I first wrote that the reminder now fires at "~25% (100,000 of 393,216)", reasoning from the flat constant this text described. The arithmetic was right and the premise had been dead for ONE DAY: `git log -S` puts the constant's removal in `08cc1b6a` (2026-08-19), **this plan's own Phase 5 Bundle B1, "a real context window, and tiers that are actually on"**. So the stale text was invalidated by our own commit the day before, and the reasoning I built on it would have re-opened a finding this plan had just closed. That is this audit's recurring defect authored by its own supervisor, caught only by grepping for a constant already quoted rather than trusting the quote — which is the entire argument for measure-then-write.
...
- 70% of the **real provider context window** (`contextTokenLimit()` → `ContextWindow::ofBackend()`; **1,048,570** on the DeepSeek-V4 sglang default as of `d97580ab`, 196,608 on MiniMax — this slot said 393,216 "as of `ed57d46a`" for a day after `d97580ab` had already moved it, so treat any number here as decayed and read the constant). **This line originally described a fixed 100,000-token proxy at `Chat::REMINDER_TOKEN_LIMIT`, `src/Chat.php:139`; that constant no longer exists anywhere in `src/` or `tests/`.** → a soft system-role text nudge is appended to the next turn (`src/Chat.php:3292-3298`), the real prompt still goes out.
...
`ProviderFactory`/`CompleteRequest` already carries a model name; each provider config can report its context window (SGLang's is **1,048,570** for the DeepSeek-V4 default and 196,608 for MiniMax as of `d97580ab` — the value is model-aware, which is exactly why a single constant could not stand in for it, and it has now decayed twice: 196,608 → 393,216 → 1,048,570, the last two within one day). **DONE, ahead of this write-up:** `Chat::contextTokenLimit()` already returns `ContextWindow::ofBackend($this->backend)` and the reminder tier consumes it, so the constant this item proposed replacing is gone. Swap the constant `Chat::REMINDER_TOKEN_LIMIT` for a value read off the active backend's provider config at construction time, so a Claude/GPT-4o session with a 128K-200K window doesn't get warned at the same fixed proxy as a smaller local model, and vice versa.
