...
| [`x/xpty`](https://github.com/charmbracelet/x/tree/main/xpty) | Pseudo-terminal primitive — open / spawn / read / write / resize a PTY pair to drive child processes | 🟡 | PR1 shipped to `candy-pty/` — `Pty::open()` round-trip via FFI to libc (`posix_openpt` + `grantpt` + `unlockpt` + `ptsname_r`), `Master` readonly DTO with idempotent `close()`, `SUGARCRAFT_LIBC` env override, Linux + macOS only (Windows ConPTY tracked separately). Spawn / resize / non-blocking I/O / signal forwarder land in PR2-PR5; `candy-wish` in-process SSH upgrade is the consumer that motivates this lib (separate plan). |
