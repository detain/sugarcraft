...
## Open review findings — 11.02

- [ ] plans/candy-serve-tui.md: ScreenStack file path is wrong — plan cites `candy-core/src/Program/ScreenStack.php` (3 occurrences in §2.2, §6 dep table, §12 references) but the actual file lives at `candy-core/src/ScreenStack.php` (no `Program/` subdir). Related types: `candy-core/src/ScreenStackCapable.php`, `candy-core/src/RootModelWithScreenStack.php`, `candy-core/src/Msg/ScreenStackPushedMsg.php`.
- [ ] plans/candy-serve-tui.md §6 dep table: ScreenStack row cites step 06.02 (correct) but PR **#581** — PR#581 is step 06.01 (Elm-style subscriptions). ScreenStack landed in **PR#583** (commit 9041563f, title "candy-core: Screen + ScreenStack types for modal workflows"). Plan should cite PR#583, not PR#581.
- [ ] plans/candy-serve-tui.md §2.4 + §6 + §12: `sugar-boxer/src/Frame.php` does not exist — sugar-boxer's primary class is `sugar-boxer/src/SugarBoxer.php` (also `Node.php`, `Lang.php`). Plan needs to update all "Frame" references to the actual class name.
- [ ] plans/candy-serve-tui.md §2.3: cites `Repo.php` methods `isPublic`, `setDescription`, `readme`. Actual surface: `isPublic` is a `readonly bool` property (not a method); `setDescription` does not exist (it's `withDescription()` at line 76 per immutable+fluent convention); `readme()` exists (line 251). Pedantic but the plan's choice of names suggests Go-style accessors not present in the PHP port.
- [ ] plans/candy-serve-tui.md §6 dep table + §5 phases 2–4: sugar-stash Git driver is listed as "✅ shipped (verify completeness)" but actual `sugar-stash/src/Git.php` only exposes `status()`, `branches()`, `log(int $limit = 25)`, `stage()`, `unstage()`. **Missing**: `tags()`, `show()`, `lsTree()`, `catFile()`. Also: `log()` signature is `(int $limit)` only — no `$ref` / `$skip` params for windowed paging. Plan correctly flags some of this as a carry-forward, but the dep table should mark sugar-stash as 🟡 (partially shipped) rather than ✅, and phase-2 should be flagged as **blocked** on sugar-stash patches landing first (not just "if missing, ship there first").
- [ ] plans/candy-serve-tui.md §5 phase 5: claims OSC 52 emitter depends on candy-vt landing one. `candy-core/src/Cmd.php:263` (`Cmd::copyToClipboard()`) already ships an OSC 52 emitter helper; candy-vt has OSC 52 *read* support but emit lives in candy-core. Plan should consume `Cmd::copyToClipboard()` directly and drop the candy-vt prerequisite (or note that the emitter already exists in candy-core).
- [ ] plans/candy-serve-tui.md §12 references list: `sugar-boxer/src/Frame.php` and `candy-core/src/Program/ScreenStack.php` both ghost paths (same as findings 1 + 3 above, repeated in the references section — needs the same fix once decided).
- [ ] plans/candy-serve-tui.md §5.1 Phase 1 file list: `candy-serve/cmd/tui.php` — `cmd/` directory does not exist (forward-looking, fine), but the alternative `bin/soft-serve` subcommand path co-exists; the plan should clarify whether `cmd/tui.php` is a new top-level convention or whether the TUI entry should live under `bin/` next to `soft-serve` per existing precedent.
